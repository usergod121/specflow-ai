...
| 人工中断 | 硬拦 | 会。界面上的「停止」→ `POST /api/cancel` → `RunService.cancel()`；引擎每轮开头问一次 `listener.cancelled()`，为真就回滚收工（`AgentResult.CANCELLED`，CLI 退出码 3）。**只在轮与轮之间生效**：正在飞的那次模型调用不会被打断，所以点完到真停可能还隔着一次调用——界面上的提示就是照这个写的 | `RunService.cancel` / `cancelled`、`AgentListener.cancelled()`、`DevelopmentAgent.execute` 开头 |
...
没有 dry-run——界面上的「运行」是直接写盘的；「停」也一样只在轮次边界生效，正在飞的那次调用拦不住。
